  ...
  it('list posts', async () => {
    await request('http://localhost:8080')
      ...
      .expect((res) => {
        ...
        expect(res.body.posts[res.body.posts.length - 1]).toMatchObject(
          context.updatedPost,
        )
      })
  })
  ...
  it('pagination', async () => {
    ...
    await request('http://localhost:8080')
      ...
      .expect((res) => {
        ...
        expect(
          res.body.posts.find((post) => post._id == context.posts[0]._id),
        ).toBeFalsy()
        expect(
          res.body.posts.find((post) => post._id == context.posts[1]._id),
        ).toBeFalsy()
      })
  })
